#!/usr/bin/env bash
# llm-council — multi-round Proposer/Critic/Implementer/Researcher debate,
# activated by the phrase `use council`.
#
# Standalone installer. Works two ways:
#   1) Cloned repo:  bash install.sh
#   2) Curl install: curl -fsSL <url>/packs/llm-council/install.sh | bash
# Pass --uninstall to remove what this installed.
set -euo pipefail

PACK_NAME="llm-council"
...
do_install() {
  ...
  info "trigger this protocol with the phrase: use council"
}
